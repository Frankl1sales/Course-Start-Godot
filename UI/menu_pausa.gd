...
func _ready() -> void:
	...
	update_icon("AspectRatioContainerBotãoMúsica/BotãoMúsica", ícone_música_normal, ícone_música_desligada_normal, not GameManager.música_desligada)
	
	$"HSliderSons".set_value_no_signal(db_to_linear(AudioServer.get_bus_volume_db(AudioServer.get_bus_index("SFX"))) * 100)
	update_icon("AspectRatioContainerBotãoSons/BotãoSons", ícone_sons_normal, ícone_sons_mudo_normal, not GameManager.sons_mutados)
