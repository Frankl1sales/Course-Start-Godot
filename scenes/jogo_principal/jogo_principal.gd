...
func _ready() -> void:   
    ...
    if GameManager.override_velocidade >= 0:
        velocidade = GameManager.override_velocidade * escala
    else:
        match GameManager.dificuldade:
            GameManager.Dificuldades.MUITO_FÁCIL, GameManager.Dificuldades.FÁCIL:
                velocidade = 0.0
            GameManager.Dificuldades.MÉDIO, GameManager.Dificuldades.MÉDIO_DIFÍCIL:
                velocidade = GameManager.VELOCIDADE_LENTA_PADRÃO * escala
            GameManager.Dificuldades.DIFÍCIL, GameManager.Dificuldades.MUITO_DIFÍCIL:
                velocidade = GameManager.VELOCIDADE_RÁPIDA_PADRÃO * escala
    ...
    for i in alvos_no_jogo.size():
        ...
        var vetor_de_movimento: Vector2 = vetor_de_movimento_aleatório()
        
        vetores_de_movimento_dos_alvos[i] = vetor_de_movimento
    
    # Não mantenha as células ocupadas se os alvos vão se mover
    if GameManager.dificuldade != GameManager.Dificuldades.MUITO_FÁCIL and \
       GameManager.dificuldade != GameManager.Dificuldades.FÁCIL:
    ...
    $CanvasLayer/Label.add_theme_font_size_override("font_size", escala * 48)
...
func clique(alvo: int) -> void:    
    ...
    if certo:        
        ...
        if GameManager.dificuldade == GameManager.Dificuldades.FÁCIL:
            trocar_alvo_de_posição(alvo_original)
        # Reposisiona o alvo móvel
        if GameManager.dificuldade == GameManager.Dificuldades.MÉDIO_DIFÍCIL or \
           GameManager.dificuldade == GameManager.Dificuldades.MUITO_DIFÍCIL:
